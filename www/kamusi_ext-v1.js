...
function displayResults(data, input_word, src_lang, trgt_lang, dumpTarget, displayEnglish, trgt_lang_word, src_lang_word) {
	...
	for (var i = 0; i < data.length; i++) {  // iterate over data sent by server
		...
		if (src_concept != null) {
			...
			var src_definition = src_concept.definition;
			var src_pos = src_concept.synset_ID_3_1;

			if (src_pos == undefined) {
				src_pos = src_concept.synset_ID_3_0;
			}
			...
			if (src_definition == undefined ) {
				src_definition = "";
				src_gloss = src_concept.gloss;
				if (src_gloss == undefined) {
					src_gloss = "";
				}
			}
			var src_example_sents  = src_concept.example_sents;
		...
		} else { // no definition for this term in database
			obj.dest_definition = "";
